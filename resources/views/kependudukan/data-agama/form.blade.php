...
@section('title', isset($item) ? 'Edit Data Agama' : 'Tambah Data Agama')
...
@php
    $isEdit = isset($item);
    $action = $isEdit ? route('kependudukan.data-agama.update', $id) : route('kependudukan.data-agama.store');
@endphp

<div class="form-header">
    <h2>{{ $isEdit ? 'Edit Data Agama' : 'Tambah Data Agama' }}</h2>
    <p>{{ $isEdit ? 'Perbarui data internal agama.' : 'Tambahkan data internal agama baru.' }}</p>
</div>
    ...
    <form action="{{ $action }}" method="POST">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" id="agama" name="agama" value="{{ old('agama', $item['agama'] ?? '') }}" required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="persentase" name="persentase" value="{{ old('persentase', $item['persentase'] ?? '0%') }}" required>
                @error('persentase')<span class="error-text">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
        ...
        <div class="form-actions">
            <a href="{{ $isEdit ? route('kependudukan.data-agama.show', $id) : route('kependudukan.data-agama.index') }}" class="btn-cancel">Batal</a>
