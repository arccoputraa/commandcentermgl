...
@section('content')
<style>
    .religion-header { display:flex; align-items:flex-start; justify-content:space-between; gap:24px; margin-bottom:38px; }
    .religion-header h2 { font-size:32px; line-height:1.2; font-weight:700; color:#1d293d; margin:0 0 14px 0; }
    .religion-header p { max-width:900px; font-size:20px; line-height:1.55; color:#708098; margin:0; }
    .add-button { height:48px; min-width:250px; margin-top:4px; border:0; border-radius:10px; background:#2563eb; color:#fff; font-size:18px; font-weight:700; display:inline-flex; align-items:center; justify-content:center; gap:14px; text-decoration:none; }
    ...
    .religion-table { width:100%; min-width:1060px; border-collapse:collapse; table-layout:fixed; }
    .religion-table th { background:#f8fafc; color:#708098; text-align:left; text-transform:uppercase; font-size:15px; line-height:1.25; font-weight:800; padding:24px 22px; border-bottom:1px solid #e5e7eb; }
    .religion-table td { color:#253044; font-size:18px; line-height:1.35; padding:24px 22px; border-bottom:1px solid #e5e7eb; vertical-align:middle; }
    .religion-table tr:last-child td { border-bottom:0; }
    .religion-table .col-no { width:55px; }
    .religion-table .col-year { width:80px; }
    .religion-table .col-area { width:145px; }
    .religion-table .col-village { width:150px; }
    .religion-table .col-religion { width:120px; }
    .religion-table .col-number { width:140px; }
    .religion-table .col-percent { width:125px; }
    .religion-table .col-date { width:145px; }
    .religion-table .col-action { width:135px; text-align:right; }
    ...
    @media (max-width:1200px) {
        .religion-header { flex-direction:column; }
        .filter-card { grid-template-columns:1fr 1fr; }
    }
    @media (max-width:760px) {
        .filter-card { grid-template-columns:1fr; }
        .add-button { width:100%; min-width:0; }
        .religion-header h2 { font-size:28px; }
        .religion-header p { font-size:17px; }
    }
</style>
...
<div class="religion-header">
    ...
    <a href="{{ route('kependudukan.data-agama.create') }}" class="add-button">
...
<form class="filter-card" action="{{ route('kependudukan.data-agama.index') }}" method="GET">
    <div class="search-field">
        <i class="fa-solid fa-magnifying-glass"></i>
        <input class="filter-input" type="search" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Cari data" aria-label="Cari data agama">
    </div>
    ...
    <select class="filter-select" name="agama" aria-label="Pilih Agama" style="color:#1d293d;">
        <option value="">Pilih Agama</option>
        @foreach($agamaOptions as $agamaOption)
            <option value="{{ $agamaOption }}" {{ ($filters['agama'] ?? '') === $agamaOption ? 'selected' : '' }}>{{ $agamaOption }}</option>
        @endforeach
    </select>
        ...
        <table class="religion-table">
            <thead>
                <tr>
                    ...
                    <th class="col-percent">Persentase</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-cell">
                                <a class="action-link view" href="{{ route('kependudukan.data-agama.show', $item['_id']) }}" aria-label="Lihat detail">
                                    <i class="fa-regular fa-eye"></i>
                                </a>
                                <a class="action-link edit" href="{{ route('kependudukan.data-agama.edit', $item['_id']) }}" aria-label="Edit data">
                                    <i class="fa-regular fa-pen-to-square"></i>
                                </a>
                                <form action="{{ route('kependudukan.data-agama.destroy', $item['_id']) }}" method="POST" style="display:inline;" onsubmit="return confirm('Hapus data agama ini?');">
                                    @csrf
                                    @method('DELETE')
