...
@section('title', isset($item) ? 'Edit Data KK' : 'Tambah Data KK')
...
@php
    $isEdit = isset($item);
    $action = $isEdit ? route('kependudukan.data-kartu-keluarga.update', $id) : route('kependudukan.data-kartu-keluarga.store');
@endphp

<div class="form-header">
    <h2>{{ $isEdit ? 'Edit Data KK' : 'Tambah Data KK' }}</h2>
    <p>{{ $isEdit ? 'Perbarui data internal kartu keluarga.' : 'Tambahkan data internal kartu keluarga baru.' }}</p>
</div>
...
<div class="form-card">
    <form action="{{ $action }}" method="POST">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                <label for="rata_rata">Rata-rata Anggota Keluarga</label>
                <input type="text" id="rata_rata" name="rata_rata" value="{{ old('rata_rata', $item['rata_rata'] ?? '0 orang') }}" required>
                @error('rata_rata')<span class="error-text">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="update">Update Terakhir</label>
                <input type="text" id="update" name="update" value="{{ old('update', isset($item) ? \Carbon\Carbon::parse($item['updated_at'])->format('d M Y') : date('d M Y')) }}" required>
        ...
        <div class="form-actions">
            <a href="{{ $isEdit ? route('kependudukan.data-kartu-keluarga.show', $id) : route('kependudukan.data-kartu-keluarga.index') }}" class="btn-cancel">Batal</a>
